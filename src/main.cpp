...
const string strMessageMagic = "testkoinz Signed Message:\n";
...
{
	RenameThread("testkoinz-loadblk");
...
bool static AlreadyHave(CTxDB& txdb, const CInv& inv)
{
	...
	case MSG_MASTERNODE_WINNER:
		return mapSeenMasternoTKZotes.count(inv.hash);
	}
		...
		const CInv &inv = *it;
		{
			...
			else if (inv.IsKnownType())
			{
				...
				if (!pushed && inv.type == MSG_MASTERNODE_WINNER) {
					if (mapSeenMasternoTKZotes.count(inv.hash)) {
						CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
						ss.reserve(1000);
						ss << mapSeenMasternoTKZotes[inv.hash];
