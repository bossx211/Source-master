// Copyright (c) 2009-2012 The Darkcoin TKZelopers
...
//
// Bootup the masternode, look for a 500 TKZ input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNoTKZin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in testkoinz.conf */
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string txHash, std::string strOutputIndex, std::string strDonationAddress, std::string strDonationPercentage, std::string& errorMessage) {
    ...
    if(!GetMasterNoTKZin(vin, pubKeyCollateralAddress, keyCollateralAddress, txHash, strOutputIndex)) {
        errorMessage = "could not allocate vin";
        LogPrintf("CActiveMasternode::Register() - Error: %s\n", errorMessage.c_str());
        return false;
    }
    CtestkoinzcoinAddress address;
...
bool CActiveMasternode::GetMasterNoTKZin(CTxIn& vin, CPubKey& pubkey, CKey& secretKey) {
	return GetMasterNoTKZin(vin, pubkey, secretKey, "", "");
}

bool CActiveMasternode::GetMasterNoTKZin(CTxIn& vin, CPubKey& pubkey, CKey& secretKey, std::string strTxHash, std::string strOutputIndex) {
    CScript pubScript;
	...
	if(!strTxHash.empty()) {
		...
		if(!found) {
			LogPrintf("CActiveMasternode::GetMasterNoTKZin - Could not locate valid vin\n");
			return false;
		}
	} else {
		...
		if(possibleCoins.size() > 0) {
			...
			LogPrintf("CActiveMasternode::GetMasterNoTKZin - Could not locate specified vin from possible list\n");
			return false;
		}
    }
...
bool CActiveMasternode::GetMasterNoTKZinForPubKey(std::string collateralAddress, CTxIn& vin, CPubKey& pubkey, CKey& secretKey) {
	return GetMasterNoTKZinForPubKey(collateralAddress, vin, pubkey, secretKey, "", "");
}

bool CActiveMasternode::GetMasterNoTKZinForPubKey(std::string collateralAddress, CTxIn& vin, CPubKey& pubkey, CKey& secretKey, std::string strTxHash, std::string strOutputIndex) {
    CScript pubScript;
	...
	if(!strTxHash.empty()) {
		...
		if(!found) {
			LogPrintf("CActiveMasternode::GetMasterNoTKZinForPubKey - Could not locate valid vin\n");
			return false;
		}
	} else {
		// No output specified,  Select the first one
		if(possibleCoins.size() > 0) {
			...
			LogPrintf("CActiveMasternode::GetMasterNoTKZinForPubKey - Could not locate specified vin from possible list\n");
			return false;
		}
    }
...
bool CActiveMasternode::GetVinFromOutput(COutput out, CTxIn& vin, CPubKey& pubkey, CKey& secretKey) {
    ...
    CtestkoinzcoinAddress address2(address1);

    CKeyID keyID;
    if (!address2.GetKeyID(keyID)) {
        LogPrintf("CActiveMasternode::GetMasterNoTKZin - Address does not refer to a key\n");
        return false;
    }

    if (!pwalletMain->GetKey(keyID, secretKey)) {
        LogPrintf ("CActiveMasternode::GetMasterNoTKZin - Private key for address is not known\n");
        return false;
    }
...
{
    CtestkoinzcoinAddress address(collateralAddress);
