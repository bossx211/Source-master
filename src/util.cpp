...
int nAnonymizetestkoinzAmount = 1000;
...
#ifdef WIN32
    // Don't need this on Linux, OpenSSL automatically uses /TKZ/urandom
...
#else
    const char* pszModule = "testkoinz";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\testkoinz
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\testkoinz
    // Mac: ~/Library/Application Support/testkoinz
    // Unix: ~/.testkoinz
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "testkoinz";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "testkoinz.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "testkoinzd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong testkoinz will not work properly.");
