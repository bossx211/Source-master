...
map<uint256, CMasternodePaymentWinner> mapSeenMasternoTKZotes;
...
void ProcessMessageMasternodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnget") { //Masternode Payments Request Sync
        ...
        if(mapSeenMasternoTKZotes.count(hash)) {
        ...
        mapSeenMasternoTKZotes.insert(make_pair(hash, winner));
    ...
    BOOST_FOREACH(CMasternodePaymentWinner& winner, vWinning){
        if(winner.nBlockHeight == winnerIn.nBlockHeight) {
            ...
            if(winner.score < winnerIn.score){
                ...
                mapSeenMasternoTKZotes.insert(make_pair(winnerIn.GetHash(), winnerIn));
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    CtestkoinzcoinAddress address4(address3);
