...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out, bool fIncludeHex)
{
    ...
    BOOST_FOREACH(const CTxDestination& addr, addresses)
        a.push_back(CtestkoinzcoinAddress(addr).ToString());
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        {
            CtestkoinzcoinAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid testkoinz address: ")+input.get_str());
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address))
        {
            entry.push_back(Pair("address", CtestkoinzcoinAddress(address).ToString()));
...
Value decoderawtransaction(const Array& params, bool fHelp)
{
    ...
    try {
        ...
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TKZ decode failed");
    }
...
Value decodescript(const Array& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", CtestkoinzcoinAddress(script.GetID()).ToString()));
    return r;
}
...
Value sendrawtransaction(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        if (!AcceptToMemoryPool(mempool, tx, true, NULL))
            throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TKZ rejected");
    }
