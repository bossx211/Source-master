...
// Copyright (c) 2009-2014 The Bitcoin TKZelopers
...
void Shutdown()
{
    ...
    RenameThread("testkoinz-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: testkoinz.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: testkoinzd.pid)") + "\n";
    ...
    strUsage += "  -regtest               " + _("Enter regression test mode, which uses a special chain in which blocks can be "
                                                "solved instantly. This is intended for regression testing tools and app TKZelopment.") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizetestkoinzamount=<n> " + _("Keep N testkoinz anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. testkoinz is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. testkoinz is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("testkoinz version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "testkoinz server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of testkoinz") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart testkoinz to complete") << "\n";
    ...
    nAnonymizetestkoinzAmount = GetArg("-anonymizetestkoinzamount", 0);
    if(nAnonymizetestkoinzAmount > 999999) nAnonymizetestkoinzAmount = 999999;
    if(nAnonymizetestkoinzAmount < 2) nAnonymizetestkoinzAmount = 2;
    ...
    LogPrintf("Anonymize testkoinz Amount %d\n", nAnonymizetestkoinzAmount);
