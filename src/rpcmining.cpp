...
Value checkkernel(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
		throw JSONRPCError(-9, "testkoinz is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(-10, "testkoinz is downloading blocks...");
...
Value mine(const Array& params, bool fHelp)
{
	...
	if (vNodes.empty())
		throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "testkoinz is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "testkoinz is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
	...
	if (params.size() > 0)
	{
		...
		const Value& moTKZal = find_value(oparam, "mode");
		if (moTKZal.type() == str_type)
			strMode = moTKZal.get_str();
		else if (moTKZal.type() == null_type)
		{
			/* Do nothing */
		}
		else
