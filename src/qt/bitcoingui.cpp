 ...
 * The Bitcoin TKZelopers 2011-2012
 */
...
{
	resize(900, 520);
	setWindowTitle(tr("testkoinz") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
	setObjectName("testkoinz");
	setStyleSheet("#testkoinz { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
	// Accept D&D of URIs
	setAcceptDrops(true);
...
void BitcoinGUI::createActions()
{
	...
	sendCoinsAction->setToolTip(tr("Send coins to a testkoinz address"));
	...
	aboutAction = new QAction(QIcon(":/icons/abouttestkoinz"), tr("&About testkoinz"), this);
	aboutAction->setToolTip(tr("Show information about testkoinz"));
	aboutAction->setMenuRole(QAction::AboutRole);
	aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
	aboutQtAction->setToolTip(tr("Show information about Qt"));
	aboutQtAction->setMenuRole(QAction::AboutQtRole);
	optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
	optionsAction->setToolTip(tr("Modify configuration options for testkoinz"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
	...
	if (clientModel)
	{
		...
		if (clientModel->isTestNet())
		{
			...
			if (trayIcon)
			{
				trayIcon->setToolTip(tr("testkoinz client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
	...
	trayIcon->setToolTip(tr("testkoinz client"));
...
void BitcoinGUI::setNumConnections(int count)
{
	...
	labelConnectionsIcon->setToolTip(tr("%n active connection(s) to testkoinz network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
	if (event->mimeData()->hasUrls())
	{
		...
		if (nValidUrisFound)
			...
			notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid testkoinz address or malformed URI parameters."));
	}
