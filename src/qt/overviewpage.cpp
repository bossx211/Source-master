...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizetestkoinzAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizetestkoinzAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizetestkoinzAmount = strAnonymizetestkoinzAmount.remove(strAnonymizetestkoinzAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizetestkoinzAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizetestkoinzAmount*COIN) nMaxToAnonymize = nAnonymizetestkoinzAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizetestkoinzAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizetestkoinzAmount));
        strAnonymizetestkoinzAmount = strAnonymizetestkoinzAmount.remove(strAnonymizetestkoinzAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizetestkoinzAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizetestkoinzAmount == 0){
