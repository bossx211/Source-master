...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizetestkoinzAmount"))
        settings.setValue("nAnonymizetestkoinzAmount", 1000);
    nAnonymizetestkoinzAmount = settings.value("nAnonymizetestkoinzAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizetestkoinzAmount"))
        SoftSetArg("-anonymizetestkoinzamount", settings.value("nAnonymizetestkoinzAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizetestkoinzAmount:
            return QVariant(nAnonymizetestkoinzAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizetestkoinzAmount:
            nAnonymizetestkoinzAmount = value.toInt();
            settings.setValue("nAnonymizetestkoinzAmount", nAnonymizetestkoinzAmount);
            emit AnonymizetestkoinzAmountChanged(nAnonymizetestkoinzAmount);
            break;
        default:
            break;
        }
