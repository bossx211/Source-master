...
void MultisigDialog::on_createAddressButton_clicked()
{
    ...
    for(int i = 0; i < ui->pubkeyEntries->count(); i++)
    {
        ...
        CtestkoinzcoinAddress address(strAddressEntered);
    ...
    CtestkoinzcoinAddress address(scriptID);
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                CtestkoinzcoinAddress address(recipient.address.toStdString());
...
void MultisigDialog::on_transaction_textChanged()
{
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        CtestkoinzcoinAddress address(addr);
...
void MultisigDialog::on_sendTransactionButton_clicked()
{
    ...
    if(fee < minFee)
    {
        QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), tr("The fee of the transaction (%1 TKZ) is smaller than the expected fee (%2 TKZ). Do you want to send the transaction anyway?").arg((double) fee / COIN).arg((double) minFee / COIN), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
    else if(fee > minFee)
    {
        QMessageBox::StandardButton ret = QMessageBox::question(this, tr("Confirm send transaction"), tr("The fee of the transaction (%1 TKZ) is bigger than the expected fee (%2 TKZ). Do you want to send the transaction anyway?").arg((double) fee / COIN).arg((double) minFee / COIN), QMessageBox::Yes | QMessageBox::Cancel, QMessageBox::Cancel);
        if(ret != QMessageBox::Yes)
            return;
    }
