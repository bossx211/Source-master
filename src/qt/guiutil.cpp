...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("testkoinz"))
        return false;
...
{
    // Convert testkoinz:// to testkoinz:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("testkoinz://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "testkoinz:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "testkoinz.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for testkoinz.lnk
...
{
    return GetAutostartDir() / "testkoinz.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a testkoinz.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=testkoinz\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the testkoinz app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add testkoinz app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("testkoinz-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  testkoinz-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("testkoinz-Qt"));
