...
const QString BITCOIN_IPC_PREFIX("testkoinz:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        socket->connectToServer(ipcServerName(), QIOTKZice::WriteOnly);
        if (!socket->waitForConnected(BITCOIN_IPC_CONNECT_TIMEOUT))
            return false;

        QByteArray block;
        QDataStream out(&block, QIOTKZice::WriteOnly);
