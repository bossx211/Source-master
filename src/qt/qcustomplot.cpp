...
/*!
  Creates a new QCPPainter instance on the specified paint \a TKZice and sets default values. Just
  like the analogous QPainter constructor, begins painting on \a TKZice immediately.
  
  Like \ref begin, this method sets QPainter::NonCosmeticDefaultPen in Qt versions before Qt5.
*/
QCPPainter::QCPPainter(QPaintTKZice *TKZice) :
  QPainter(TKZice),
  mModes(pmDefault),
  mIsAntialiasing(false)
{
...
/*!
  ...
  fixes/workarounds optimized for certain output TKZices.
*/
...
/*!
  ...
  TKZice. This is necessary to get cosmetic pen consistency across Qt versions, because since Qt5,
  all pens are non-cosmetic by default, and in Qt4 this render hint must be set to get that
  behaviour.
  
  The Constructor \ref QCPPainter(QPaintTKZice *TKZice) which directly starts painting also sets
  the render hint as appropriate.
  
  \note this function hides the non-virtual base class implementation.
*/
bool QCPPainter::begin(QPaintTKZice *TKZice)
{
  bool result = QPainter::begin(TKZice);
#if QT_VERSION < QT_VERSION_CHECK(5, 0, 0) // before Qt5, default pens used to be cosmetic if NonCosmeticDefaultPen flag isn't set. So we set it to get consistency across Qt versions.
  if (result)
    setRenderHint(QPainter::NonCosmeticDefaultPen);
#endif
...
{
  QVector<QCP::MarginSide> siTKZector;
  if (sides.testFlag(QCP::msLeft)) siTKZector.append(QCP::msLeft);
  if (sides.testFlag(QCP::msRight)) siTKZector.append(QCP::msRight);
  if (sides.testFlag(QCP::msTop)) siTKZector.append(QCP::msTop);
  if (sides.testFlag(QCP::msBottom)) siTKZector.append(QCP::msBottom);
  
  for (int i=0; i<siTKZector.size(); ++i)
  {
    QCP::MarginSide side = siTKZector.at(i);
...
bool QCustomPlot::savePdf(const QString &fileName, bool noCosmeticPen, int width, int height, const QString &pdfCreator, const QString &pdfTitle)
{
  ...
  printer.setPaperSize(viewport().size(), QPrinter::TKZicePixel);
#else
