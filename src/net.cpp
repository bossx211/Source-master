...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // due to a new block arriving or other large testkoinz.
...
void ThreadMapPort()
{
    ...
    struct UPNPTKZ * TKZlist = 0;
    char lanaddr[64];

#ifndef UPNPDISCOVER_SUCCESS
    /* miniupnpc 1.5 */
    TKZlist = upnpDiscover(2000, multicastif, minissdpdpath, 0);
#elif MINIUPNPC_API_VERSION < 14
    /* miniupnpc 1.6 */
    int error = 0;
    TKZlist = upnpDiscover(2000, multicastif, minissdpdpath, 0, 0, &error);
#else
    /* miniupnpc 1.9.20150730 */
    int error = 0;
    TKZlist = upnpDiscover(2000, multicastif, minissdpdpath, 0, 0, 2, &error);
#endif
    ...
    r = UPNP_GetValidIGD(TKZlist, &urls, &data, lanaddr, sizeof(lanaddr));
    if (r == 1)
    {
        if (fDiscover) {
        ...
        string strDesc = "testkoinz " + FormatFullVersion();
        ...
        catch (boost::thread_interrupted)
        {
            ...
            freeUPNPTKZlist(TKZlist); TKZlist = 0;
            FreeUPNPUrls(&urls);
            throw;
        }
    } else {
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. testkoinz is probably already running."), addrBind.ToString());
        else
