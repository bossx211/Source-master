...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop testkoinz server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "testkoinz server stopping";
}
...
private:
    SSLIOStreamTKZice<Protocol> _d;
    iostreams::stream< SSLIOStreamTKZice<Protocol> > _stream;
};
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        string strWhatAmI = "To use testkoinzd";
...
std::string HelpExampleCli(string methodname, string args){
    return "> testkoinzd " + methodname + " " + args + "\n";
}
